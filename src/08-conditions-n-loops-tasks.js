...
function doRectanglesOverlap(rect1, rect2) {
  ...
  // if (rect1.top + rect1.width >= rect2.top) return true;
  return true;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const f1 = isStartIncluded ? '[' : '(';
  const f4 = isEndIncluded ? ']' : ')';
  const f2 = a < b ? a : b;
  const f3 = a < b ? b : a;
  return `${f1}${f2}, ${f3}${f4}`;
}
...
function isBracketsBalanced(str) {
  ...
  str.split('').forEach((el) => {
    const lastInStack = stack[stack.length - 1] || null;
    if (map[lastInStack] === el) {
      stack.pop();
    } else {
      stack.push(el);
    }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 1; i < f1.length; i += 1) {
    for (let j = 0; j < f1[i].length; j += 1) {
      if (f1[i][j] !== res[j]) {
        res = res.slice(0, j);
      }
    }
